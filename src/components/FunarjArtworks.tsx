...
export function FunarjArtworks() {
  ...
  const LocationPinRow = ({ iconSize, fontSize, gap, topOffset, textOffset, tracking }: { iconSize: number, fontSize: number, gap: number, topOffset: number, textOffset: number, tracking: string }) => (
    <div className="relative flex" style={{ paddingLeft: `${iconSize + gap}px`, color: BSN_NAVY, height: `${fontSize}px` }}>
      <div className="absolute left-0" style={{ top: `${topOffset}px` }}>
        <MapPin size={iconSize} strokeWidth={2.5} />
      </div>
      <span className="font-black uppercase block leading-none" style={{ fontSize: `${fontSize}px`, letterSpacing: tracking, paddingTop: `${textOffset}px` }}>
        TEATRO JOÃO CAETANO
      </span>
    </div>
  );
  ...
  const handleDownloadPDF = async (id: string, filename: string) => {
    ...
    try {
      ...
      // Add image taking the full A4 page (210 x 297 mm)
  ...
  return (
    <div className="w-full min-h-screen bg-neutral-900 flex flex-col items-center py-12 px-4 gap-16 font-sans">
      ...
      <div className="text-center mb-8 text-white">
        ...
        <p className="text-white/60 text-sm mb-6">Site de Vendas: Estilo Swiss / Bandeira do Brasil</p>
        
        <label className="inline-flex items-center cursor-pointer bg-white/10 px-5 py-2.5 rounded-full border border-white/20 hover:bg-white/20 transition-colors shadow-lg">
          <input 
            type="checkbox" 
            className="sr-only" 
            checked={showDetails} 
          ...
          />
          <div className={`w-10 h-6 rounded-full p-1 transition-colors ${showDetails ? 'bg-[#006b3f]' : 'bg-white/30'}`}>
            <div className={`bg-white w-4 h-4 rounded-full shadow-md transform transition-transform ${showDetails ? 'translate-x-4' : 'translate-x-0'}`} />
          </div>
          <span className="ml-3 text-white font-bold text-sm tracking-wide">
            {showDetails ? "Modo Completo (Logos e Datas)" : "Modo Limpo (Apenas Arte)"}
          </span>
        </label>
      </div>
      ...
      <div className="flex flex-col gap-24 items-center w-full" ref={containerRef}>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          >
            ...
            <div className={`w-[260px] flex flex-col border-r-4 border-[#002776] ${showDetails ? 'h-[420px]' : 'h-[490px]'}`}>
              {GUESTS.map((guest, idx) => (
                <div key={idx} className="flex-1 flex flex-col p-4 border-b-2 justify-center relative overflow-hidden shrink-0" style={{ background: guest.bg, borderColor: BSN_NAVY }}>
            ...
            <div className={`w-[600px] flex flex-col relative shrink-0 ${showDetails ? 'h-[420px]' : 'h-[490px]'}`}>
              ...
              <div className={`flex p-6 gap-6 shrink-0 w-full ${showDetails ? 'h-[220px]' : 'h-[290px]'}`}>
                
                {/* Left Side of Bottom (Orchestra Photo) */}
                <div className={`h-full transition-all duration-300 ${showDetails ? 'w-[45%]' : 'w-[58%]'}`}>
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {showDetails && (
                      <>
                        <LocationPinRow iconSize={14} fontSize={13} gap={5} topOffset={-2} textOffset={2} tracking="0.2em" />
            ...
            {showDetails && <SponsorBar height={70} />}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          >
            {/* Left Sidebar: Guests (30% width) */}
            <div className={`w-[140px] flex flex-col border-r-2 border-[#002776] ${showDetails ? 'h-[280px]' : 'h-[330px]'}`}>
              {GUESTS.map((guest, idx) => (
                <div key={idx} className="flex-1 flex flex-col p-2.5 border-b-2 justify-center relative overflow-hidden shrink-0" style={{ background: guest.bg, borderColor: BSN_NAVY }}>
            ...
            <div className={`w-[360px] flex flex-col relative shrink-0 ${showDetails ? 'h-[280px]' : 'h-[330px]'}`}>
              ...
              {/* MIDDLE / BOTTOM: TITLE & DETAILS */}
              <div className={`w-full flex p-4 gap-4 shrink-0 ${showDetails ? 'h-[160px]' : 'h-[210px]'}`}>
                
                <div className={`h-full transition-all duration-300 ${showDetails ? 'w-[45%]' : 'w-[55%]'}`}>
                ...
                <div className="flex-1 flex flex-col justify-center">
                  ...
                  <div className="flex flex-col gap-1">
                    {showDetails && (
                      <>
                        <LocationPinRow iconSize={10} fontSize={9} gap={3} topOffset={-1} textOffset={1} tracking="0.1em" />
            ...
            {showDetails && <SponsorBar height={50} />}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          >
            ...
            <div className={`w-full flex p-4 gap-3 shrink-0 ${showDetails ? 'h-[200px] pb-12' : 'h-[240px] pb-4'}`}>
              ...
              <div className="w-[140px] h-full flex flex-col justify-between shrink-0">
                ...
                <div className="flex flex-col items-center text-center gap-0.5">
                  {showDetails && (
                    <>
                      <LocationPinRow iconSize={9} fontSize={8} gap={2} topOffset={-1} textOffset={0} tracking="0.1em" />
            ...
            {showDetails && <SponsorBar height={40} />}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          ...
          >
             ...
             <div className={`w-full flex p-5 gap-5 shrink-0 ${showDetails ? 'h-[240px]' : 'h-[300px]'}`}>
               {/* Left: Band Photo */}
               <div className={`h-full border-4 relative overflow-hidden p-1 bg-white shrink-0 transition-all duration-300 ${showDetails ? 'w-[50%]' : 'w-[62%]'}`} style={{ borderColor: BSN_NAVY }}>
                 <BandPhoto />
               </div>
               {/* Right: Title & Date */}
               <div className="flex-1 flex flex-col justify-center shrink-0 pr-4">
                  ...
                  <div className="flex flex-col gap-1.5">
                    {showDetails && (
                      <>
                        <LocationPinRow iconSize={12} fontSize={11} gap={4} topOffset={-1} textOffset={1} tracking="0.1em" />
                        <div className="mt-1 flex items-center gap-1.5">
             ...
             {showDetails && <SponsorBar height={60} />}
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="flex items-center gap-6">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => handleDownloadPDF('art-5', 'Funarj_Cartaz_420x594')} className="px-4 py-1.5 bg-white text-[#002776] text-xs font-black uppercase tracking-wider hover:bg-gray-200 transition-colors cursor-pointer rounded-sm border border-[#002776] shadow-sm">
                ⬇ Baixar PDF
              </button>
            </div>
          </div>
          ...
          >
             ...
             <div className={`w-[100px] flex flex-col border-r-4 shrink-0 ${showDetails ? 'h-[544px]' : 'h-[594px]'}`} style={{ borderColor: BSN_NAVY }}>
             ...
             <div className={`w-[320px] flex flex-col shrink-0 ${showDetails ? 'h-[544px]' : 'h-[594px]'}`}>
               {/* BAND PHOTO */}
               <div className={`w-full p-4 shrink-0 border-b-4 transition-all duration-300 ${showDetails ? 'h-[180px]' : 'h-[290px]'}`} style={{ borderColor: BSN_NAVY }}>
                 <div className="w-full h-full border-4 relative overflow-hidden p-1 bg-white" style={{ borderColor: BSN_NAVY }}>
                    <BandPhoto />
                 </div>
               </div>
               ...
               {/* TITLE & DETAILS */}
               <div className={`w-full pl-5 pr-4 flex items-center justify-between border-b-4 shrink-0 transition-all duration-300 ${showDetails ? 'h-[140px]' : 'h-[80px]'}`} style={{ borderColor: BSN_NAVY }}>
                 <div className="flex flex-col flex-1 justify-center h-full py-3">
                   ...
                   <div className="flex flex-col gap-1">
                     {showDetails && (
                       <>
                         <LocationPinRow iconSize={11} fontSize={10} gap={3} topOffset={-1} textOffset={1} tracking="0.1em" />
                         <div className="mt-1 flex items-center gap-1.5">
                           <DateBadge text="24 JUL" width={42} height={14} fontSize={10} yOffset={11} />
                 ...
                 {showDetails && (
                   <div className="ml-4 pl-4 border-l-[1.5px] flex flex-col items-center shrink-0 justify-center h-[100px]" style={{ borderColor: BSN_NAVY }}>
                     <div className="bg-white p-1 border-[1.5px]" style={{ borderColor: BSN_NAVY }}>
                       <QRCode value="https://funarj.eleventickets.com/#!/apresentacao/92246ebabb92ec4abc06a31662b8937491dacc5a" size={54} fgColor={BSN_NAVY} level="M" />
                     </div>
                     <span className="text-[7px] font-black tracking-widest mt-1.5" style={{ color: BSN_NAVY }}>INGRESSOS</span>
                   </div>
                 )}
               </div>
